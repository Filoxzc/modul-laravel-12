...
File `routes/api.php` lalu dibuat berisi seluruh endpoint yang diperlukan :
...
Empat Controller API dibuat di folder terpisah `Api/` (bukan bercampur dengan Controller web), begitu juga tiga API Resource:
...
`destroy()` menghapus buku dan merespon dengan pesan konfirmasi sederhana - tidak memakai API Resource karena tidak ada lagi data buku yang perlu diformat setelah dihapus:
...
Sesuai daftar endpoint yang ditampilkan, `members` hanya butuh `GET` (list dan detail) - tidak ada create/update/delete lewat API di pertemuan ini:
...
Endpoint `kembalikan` - yang di versi web (`LoanController@kembalikan`) masih sengaja dibiarkan sebagai Tugas mandiri Pertemuan 7 - di sisi API justru diimplementasikan penuh karena secara eksplisit diminta di daftar endpoint pertemuan ini:
